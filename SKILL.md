...
description: 音频转录精炼全流程。从网络下载音频（B站/网盘/直链），使用本地 Qwen3-ASR 进行中文语音识别（GPU加速），自动生成带时间戳的原始转录和按主题整理的精炼版。支持断点续传、智能重叠去重、全平台GPU加速、完整性验证。触发词：转录录音、整理录音、音频转文字、视频转文稿、下载转录、会议纪要、录音整理、播客转录、语音识别、ASR。
---

# 🎙️ Audio Transcription Skill

> GitHub: https://github.com/dongmao/audio-transcription
...
- **全平台 GPU**：Intel XPU (Arc) / NVIDIA CUDA 自动检测
...
┌──────────────────────────────────────────────────────────────────┐
│  Phase 1: 环境检测（一次性）                                       │
│  → 检测 GPU 类型 (Intel XPU / NVIDIA CUDA / CPU)                 │
...
# Playwright 浏览器（网盘下载必需，使用系统 Edge 无需额外下载 Chromium）
# 如果需要 Chromium: python -m playwright install chromium
...
```powershell
# 自动识别来源类型
python scripts/download_audio.py "<URL>" -o "录音.mp3"
...
**B站视频/直播：**
```powershell
python -m yt_dlp -x --audio-format mp3 --audio-quality 0 "<URL>" -o "录音.mp3"
```

**网盘分享（极空间/百度/阿里云盘等）：**

网盘分享页面是 SPA 单页应用，需要浏览器渲染。使用 Playwright + 系统 Edge 浏览器自动下载：
...
```powershell
python scripts/transcribe_qwen3_asr.py "录音.mp3" [options]
```
...
```powershell
# 基本用法（自动检测GPU）
python scripts/transcribe_qwen3_asr.py "录音.mp3"

# 指定 Intel XPU
python scripts/transcribe_qwen3_asr.py "录音.mp3" -d "xpu:0"

# 恢复中断的转录（自动读取 checkpoint）
python scripts/transcribe_qwen3_asr.py "录音.mp3" -o "之前的输出.txt"
```
...
> **这是本 skill 最核心的部分**。详细指南见 `references/精炼版整理指南.md`，示例见 `references/精炼版示例.md`。
...
| `references/精炼版示例.md` | 实际精炼版样本，供学习参考 |
