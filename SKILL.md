...
│  → 按主题重组章节，标注时间锚定                                   │
│  → 对照专有名词表修正ASR识别错误                                  │
│  → 不确定的标注 [待确认]，末尾附校正表 + 术语表                   │
│  → 去除口语冗余，保留论证过程                                     │
│  → 结构化数据用表格，信息分层组织                                 │
│  → 文头元信息使用表格格式（含关键词字段）                         │
│  → 文头后紧跟摘要章节，提炼核心结论与适用场景                      │
│  → 文末关键数据速览                                               │
│  → 完整性验证：段数 + 时间覆盖双重校验                           │
│                                                                  │
│  Phase 5: 交付                                                    │
│  → 精炼版 + 原始转录合并为单个文件，精炼版在前、原始版在后         │
│  → 原始转录时间戳加粗对齐，关键概念/指标/公司名加粗突出            │
│  → 文件名格式：YYYYMMDD_类别_关键主体_完整版.md                   │
│  → 完整性验证通过后交付                                           │
└──────────────────────────────────────────────────────────────────┘
```
...
2. **表格/图表优先**——多维度对比、时间线、分类数据必须用表格展示，每个精炼版至少5-10个表格+示意图
3. **信息密度优先**——表格为了更高信息密度，不为做而做
4. **时间锚定**——每个章节标注原始音频时间区间
5. **完整性可验证**——精炼版必须覆盖原始转录 100% 内容
...
**何时用表格（2026-06-13 强化版：必须主动寻找可表格化的内容）**：
- ✅ **必须用表格**：多维度对比（3+维度）、时间线/阶段演进、分层结构、多方观点并列、分类数据
- ✅ **优先用表格**：任何涉及2个以上对象的比较、任何5行以上的结构化数据
- ❌ 一句话能说清、叙述性内容、单一数值、情感/语气描述
- ⚠️ **检查标准**：如果一段话超过5行并涉及2个以上对象的对比，**必须转为表格**
- ⚠️ **每个精炼版至少5-10个表格/图示**，否则说明替代理由
...
| 文件 | 精炼版 + 原始转录**合并为单个文件**，精炼版在前、原始版在后，用 `---` 分隔 |
| 文件名 | `YYYYMMDD_类别_关键主体_完整版.md` |
| 文件名示例 | `20260617_DD_无人机纯视觉TechDD_完整版.md` |
| 类别标签 | `DD`=尽调, `访谈`=访谈, `纪要`=会议纪要, `研报`=研究, `播客`=播客 |
| 文头格式 | 表格形式，含转录模型/时间/设备/来源/时长/性质/参会方/关键词/段数/验证 |
| 摘要 | 文头后紧跟摘要章节，提炼核心结论 + 适用场景 |
| 术语表 | 精炼版末尾（ASR校正表之后），含术语/英文/简要说明 |
| 原始转录 | 时间戳 `**[MM:SS]**` 加粗对齐，关键概念/指标/公司名加粗突出 |
| 编码 | UTF-8 |
| 交付方式 | 使用 present_files 工具交付单个合并文件 |
...
- [ ] **⭐ 精炼版末尾附有 ASR识别校正表**
- [ ] 交付文件为 .md 格式（非 .txt）
- [ ] 文件名为单个合并文件，精炼版在前、原始版在后
- [ ] 文头元信息完整
- [ ] **⭐ 精炼版深度自检：没听原录音的人能完全搞懂吗？**
- [ ] **⭐ 表格充足：至少5-10个表格/图示，覆盖对比/时间线/多维度数据**（如没有，说明用纯文本替代的理由）
...
### 教训十五：精炼版必须有表格/图形/示意图来展示对比和结构（⭐ 2026-06-13 新增）

**现象**：精炼版虽然有内容深度，但全是段落叙述，没有表格、对比图或示意图。用户反馈"怎么一个表格或者图形都没有了"。

**根因**：AI 倾向于用文字叙述，忽略了**表格是最高效的信息呈现方式**——尤其是对比、时间线、多维度并列的场景。纯文本的精炼版阅读体验差，读者需要自己在脑中"拼图"。

**精炼版中必须包含表格的场景**（每个精炼版至少应有5-10个表格/图示）：

| 内容类型 | 建议的表格/图形 | 示例 |
|---------|---------------|------|
| 概念对比 | 对比表（多维度） | 五系 vs 七系（10维度：核心任务/道路类型/风险敞口/典型行为） |
| 时间演进 | 阶段对比表 | 机器人三阶段（空间/技术/状态/市场判断） |
| 竞争分析 | 梯队/排名表 | 三层护城河（层级/评分/逻辑/案例） |
| 技术架构 | 对比图或推理链 | 冯诺依曼 vs 存算一体（搬运路径可视化） |
| 产品进化 | 阶段递进表 | 产品智能三阶段：不可调→可调节→真正智能 |
| 业务布局 | 分类表 | 四大业务方向（品类/七系进展/市场判断） |
| 关键数据 | 数据速览表（已有） | 市值/员工/Token/费用/工程师数 |

**表格设计原则**（2026-06-13 验证）：
- 每个表格应有**明确的对比维度**（列标题清晰）
- 文字段落和表格**交替穿插**——段落解释背景+表格总结对比，不堆叠
- 重要的数字/评分用**加粗**突出
- 3-4个对比维度是最佳平衡，一个维度一行
- 表格不应替代叙述，而是**补充叙述**——读者先看段落理解背景，再看表格抓住对比

**正确做法**：
- 每章至少检查一遍：这段内容是否可以用表格来展示？
- 优先考虑表格：多维度数据、对比关系、时间线、分类
- 示例验证（2026-06-13，张小珺访谈阳萌37KB精炼版）：用户认可的版本有9个表格+3幅示意图
- 检查标准：如果一段话超过5行并涉及2个以上对象的对比→必须转为表格

**实战数据**：
- 阳萌访谈精炼版V1：仅有3个表格（一三五七/第三类公司/关键数据）→ 用户不满意
- 阳萌访谈精炼版V2：9个表格（五系vs七系/分治vs端到端/机器人三阶段/业务方向/三层护城河/产品智能三阶段/关键数据/一三五七/第三类公司）+ 3幅示意图 → 用户认可
